        ...
        for i in VIDEO_SETS[split]:
            ...
            for j, image_name in enumerate(image_files):
                if (j < image_range[0] or j > image_range[1]):
                    continue
                num_images += 1
                toRead = os.path.join(video_path, image_name)
                print(toRead)
                image = cv2.imread(toRead, cv2.IMREAD_UNCHANGED)
            ...
            for ann_ind, txt in enumerate(anns):
                ...
                bbox = _bbox_to_coco_bbox(bbox)  # coco format [x,y,width,height]
                area = bbox[2] * bbox[3]
