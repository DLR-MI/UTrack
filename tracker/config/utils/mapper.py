...
class Mapper(object):
    ...
    def mapto(self, box, var_box, uncertain):
        uv = np.array([[box[0]+box[2]/2], [box[1]+box[3]]])
        if uncertain:
            var_uc, var_vc, _, var_h = var_box
            # Center and bottom center horizontal has the same var
            u_err = np.sqrt(var_uc)
            # Center and bottom center vertical are shifted
            v_err = np.sqrt(var_vc + 0.25*var_h) # no Cov(vc, h)
        else:
            # Default in UCMCTrack
